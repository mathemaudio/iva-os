...
export class ShellWallpaperCatalogTest {
	...
	static async resolvesBackgroundStyles(scenario: ScenarioParameter): Promise<{ builtInUsesGradient: boolean, vfsUsesImageUrl: boolean, fallbackUsesAurora: boolean }> {
		...
		const builtInStyle = ShellWallpaperCatalog.getBackgroundStyle('grid', snapshot)
		const vfsStyle = ShellWallpaperCatalog.getBackgroundStyle(createdWallpaper.id, snapshot)
		const fallbackStyle = ShellWallpaperCatalog.getBackgroundStyle('missing-node', snapshot)
